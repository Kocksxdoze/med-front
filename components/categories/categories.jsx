...
function Categories() {
  ...
  const handleCreatecategory = async () => {
    try {
      if (isEditing) {
        await axios.put(
          `http://0.0.0.0:4000/category/edit/${editingId}`,
          formData
        );
        toast({
        ...
        await axios.post("http://0.0.0.0:4000/category/new", formData);
        toast({
          title: "Филиал создан.",
          status: "success",
          duration: 3000,
  ...
  const handleDeletecategory = async (id) => {
    try {
      await axios.delete(`http://0.0.0.0:4000/category/delete/${id}`);
      toast({
        title: "Филиал удалён.",
        status: "success",
        duration: 3000,
