...
function AppTypes() {
  ...
  async function loadbases() {
    const data = await fetcher("types");
  ...
  const handleCreatebase = async () => {
    try {
      if (isEditing) {
        await axios.put(
          `http://localhost:4000/type/update/${editingId}`,
          formData
        );
        toast({
          title: "Тип обновлён.",
        ...
        await axios.post("http://localhost:4000/type/new", formData);
        toast({
          title: "Тип создан.",
          status: "success",
          duration: 3000,
      ...
      toast({
        title: "Ошибка при создании типа.",
  ...
  const handleDeleteBase = async (id) => {
    try {
      await axios.delete(`http://localhost:4000/type/delete/${id}`);
      toast({
        title: "Тип удалён.",
        status: "success",
        duration: 3000,
      ...
      console.error("Ошибка при удалении Типаа:", error);
    }
  };
  ...
  return (
    <Box p={4} borderRadius="16px" w="100%" overflowX="auto" bg="#fff">
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="2xl">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Создание Тип</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            <FormControl mb={3}>
              <FormLabel>Имя типа встреч</FormLabel>
              <Input
                value={formData.name}
                onChange={(e) =>
            ...
            <FormControl mb={3}>
              <FormLabel>Описание типа встреч</FormLabel>
              <Input
                value={formData.desc}
                onChange={(e) =>
