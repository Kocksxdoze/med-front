...
function Benefit() {
  ...
  const handleCreatebase = async () => {
    try {
      if (isEditing) {
        await axios.put(
          `http://0.0.0.0:4000/benefit/update/${editingId}`,
          formData
        );
        toast({
        ...
        await axios.post("http://0.0.0.0:4000/benefit/new", formData);
        toast({
          title: "Льгота создан.",
          status: "success",
          duration: 3000,
  ...
  const handleDeleteBase = async (id) => {
    try {
      await axios.delete(`http://0.0.0.0:4000/benefit/delete/${id}`);
      toast({
        title: "Льгота удалён.",
        status: "success",
        duration: 3000,
