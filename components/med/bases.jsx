...
function Bases() {
  ...
  const handleCreatebase = async () => {
    try {
      if (isEditing) {
        await axios.put(
          `http://192.168.1.13:4000/base/edit/${editingId}`,
          formData
        );
        toast({
        ...
        await axios.post("http://192.168.1.13:4000/base/new", formData);
        toast({
          title: "Филиал создан.",
          status: "success",
          duration: 3000,
  ...
  const handleDeleteBase = async (id) => {
    try {
      await axios.delete(`http://192.168.1.13:4000/base/delete/${id}`);
      toast({
        title: "Филиал удалён.",
        status: "success",
        duration: 3000,
  ...
  return (
    <Box p={4} borderRadius="16px" w="100%" overflowX="auto" bg="#fff">
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="2xl">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Создание Филлиала</ModalHeader>
          <ModalCloseButton />
          <ModalBody pb={6}>
            {[{ name: "name", label: "Имя филлиала" }].map(
              ({ name, label }) => (
