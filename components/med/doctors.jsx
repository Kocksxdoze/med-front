...
function Doctors() {
  ...
  const handleSaveDoctor = async () => {
    try {
      if (formData.id) {
        await axios.put(
          `http://192.168.1.13:4000/doctor/edit/${formData.id}`,
          formData
        );
        toast({
        ...
        await axios.post("http://192.168.1.13:4000/register", formData);
        toast({
  ...
  const handleDeleteDoctor = async (id) => {
    try {
      await axios.delete(`http://192.168.1.13:4000/doctor/delete/${id}`);
      toast({
        title: "Врач удалён.",
        status: "success",
        duration: 3000,
