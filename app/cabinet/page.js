...
function Cabinet() {
  ...
  const handleSave = () => {
    ...
    if (userRole === "laboratory") {
      ...
      endpoint = `http://192.168.1.13:4000/lab/update/${selectedLabId}`;
      ...
      if (saveTarget === "diagnosis") {
        ...
        endpoint = `http://192.168.1.13:4000/dia/update/${selectedDiagnosisId}`;
        ...
        endpoint = `http://192.168.1.13:4000/offer/update/${selectedOfferId}`;
    ...
    fetch(endpoint, {
      ...
      .then(() => {
        ...
        closeModal();
        return fetch("http://192.168.1.13:4000/clients")
          .then((res) => res.json())
          .then((data) => {
            const filteredClients = data.filter(
              (client) => client.doctorId === doctor.id
