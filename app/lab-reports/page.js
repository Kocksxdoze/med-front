  ...
  const fetchLabReports = async () => {
    try {
      ...
      const labCategory = allCategories.find((cat) =>
        cat.name.toLowerCase().includes("лабораторный")
      );

      if (!labCategory) {
        toast({
          title: "Лабораторная категория не найдена",
          status: "warning",
          duration: 3000,
          isClosable: true,
        });
      ...
      <Flex flexDir={"column"} pos={"absolute"} w={"100%"}>
        ...
        >
          ...
          >
            ...
            >
              {loading ? (
                <Text>Загрузка лабораторных отчетов...</Text>
              ) : labCategory ? (
                <>
                            ...
                            <Td colSpan={4} textAlign="center" py={8}>
                              <Alert status="info" borderRadius="md">
                                ...
                                {searchTerm
                                  ...
                                  : "Нет доступных лабораторных отчетов"}
                              </Alert>
                            </Td>
                          </Tr>
                        )}
                ...
                <Alert status="warning" borderRadius="md">
                  <AlertIcon />
                  Лабораторная категория не найдена
                </Alert>
              )}
            </Box>
          </Box>
      ...
      >
        ...
        <ModalContent maxW="90vw" maxH="90vh">
          <ModalHeader fontSize="xl" bg="blue.500" color="white">
            {selectedReport?.name || "Лабораторный отчёт"}
